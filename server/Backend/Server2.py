...
def clean_insights(insights):
    """Removes <think>...</think> and asterisks (*) from AI-generated insights."""
    insights = re.sub(r"<think>.*?</think>", "", insights, flags=re.DOTALL)
    insights = insights.replace("*", "")
    insights = insights.encode("utf-8").decode("unicode_escape")
    insights = insights.replace("\\n", "\n")
    # insights = insights.encode().decode('unicode_escape')
    # insights = re.sub(re.escape("\n"), '\n', insights)
...
def generate_ai_insights(region):
    ...
    Just **directly output structured urban planning insights with concrete numbers.**  
    Create a blueprint that will help urban planners create future ready cities that enhance the quality of life of residents.
    Include the following insights for the region of **{region}**:
    {{
      "Traffic & Transportation": {{
        "Average Speed (km/h)": "[Use {traffic_data}]",
        "Peak Congestion Time": "[Provide {region} specific]",
        "Major Congestion Points": [Provide {region} specific],
        "Solutions": [Provide {region} specific]
      }},
      "Energy Consumption & Sustainability": {{
        "Per Capita Energy (kWh)": "[Provide {region} specific number]",
        "Renewable Energy (%)": "[Provide {region} specific percentage]",
        "Solar Feasibility": "Provide {region} specific",
        "Energy Wastage Reduction": [Provide {region} specific]
      }},
      "Environmental Impact": {{
        "Air Pollution (AQI)": "[Use {pollution_data}]",
        "Noise Pollution (dB)": "[Provide {region} specific]",
        "Temperature ": "[Use {weather_data} and use in degree Celcius]" 
        "Green Cover (%)": "[Provide {region} specific percentage]",
        "Sustainability Strategies": [[Provide {region} specific number]
      }},
      "Demographics & Infrastructure Needs": {{
        "Population Density (people/sq. km)": "[Use {population_data}]",
        "Hospitals Needed": "[Estimate {region} specific count between 0-5]",
        "Schools Needed": "[Estimate {region} specific count between 0-5]",
        "Infrastructure Improvements": [Provide {region} specific improvements]
      }}
    }}
    **OUTPUT THE ABOVE IN TEXT FORMAT AND USE THOSE CONCRETE NUMBERS IN TEXT TO GIVE INSIGHTS.**
    """
    messages = [
        {'role': 'system', 'content': "Provide structured, data-driven urban planning insights for Mumbai with concrete numbers."},
    ...
    raw_insights = completion.choices[0].message.content.strip()
    cleaned_insights = clean_insights(raw_insights)  # Apply cleaning function
...
# Route to process region and generate insights
...
def generate_insights():
    try:
        ...
        region = data['query']  # Get the region directly since we checked for 'query'

        if not region:
            return jsonify({"error": "Region is required"}), 400

        insights = generate_ai_insights(region) or "No insights available."
